...
function Sidebar({ retirementAge,
  ...
  const [desiredAnnualIncomeHelperText, setDesiredAnnualIncomeHelperText] = useState('');
  ...
  const handleInputChangeDesiredAnnualIncome = (event) => {
    const newValue = event.target.value;
    const isPositive = parseFloat(newValue) >= 0 || newValue === '';

    setValue(newValue);

    if (!isPositive) {
      setError(true);
      setDesiredAnnualIncomeHelperText('Please enter a positive number');
    } else {
      setError(false);
      setHelperText('');
    }
            ...
            <Stack spacing={2}> {/* Stack components vertically with spacing */}
              ...
              {graphType === "income" && (
                <TextField
                  ...
                  onChange={(e) => onUpdate({ desiredAnnualIncome: Number(e.target.value) })}
                  fullWidth
                  error={error}
                  helperText={desiredAnnualIncomeHelperText}
                  variant="outlined"
                  size="small"
                  InputProps={{
